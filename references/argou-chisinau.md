# Argou chișinăuian — listă suplimentară (mod stradal, opt-in)

> **Registru:** argou urban chișinăuian, puternic rusificat. Compilație de
> internet — **sursă neserioasă**, păstrată ca atare.
>
> **Cum se aplică:** numai cînd e invocat explicit (`/omenizator --argou` sau
> „dă-i pe stradal"). Implicit, skill-ul folosește modul **literar** (DMR/Stati)
> și NU atinge lista de mai jos.
>
> **⚠ = vulgar/sexual** — exclus implicit din output; intră doar la cerere
> explicită de registru licențios.

## A
- a abaldi — a rămîne cu gura căscată
- a abijî — a ofensa
- a abișci — a promite
- a afighi — a rămîne uimit
- a ahui — a încălca regulile de conduită
- a buksui — a patina cu mașina / a nu face nimic
- a chica pe buși — a fi uimit
- a dolojî cuiva șeva — a da raportul / a spune
- a da atciotu — a face darea de seamă
- a hriukni — a scoate sunete din nas
- a fași lâjâli — a te duce
- a fași perevarot — a face revoluție
- a fași nazlo — a face din ciudă
- a naezji — a avea pretenții
- a pastupi jestoka — a te purta dur
- a peredăi privet — a transmite salutări
- a perejîvăi — a-ți face griji
- a pridumăi — a inventa ceva
- a pune lapșa pi urechi — a minți
- a pasfiti — a dedica
- a umbla cu teolka — a te plimba cu prietena
- a sări în skakalcă — a sări cu coarda
- a sobiri prikindalili — a strînge lucrurile
- a stuki — a scuipa
- a sineaci — a consuma alcool în cantități mari
- ⚠ a te zaibi — a te sătura
- a te snimi — a fugi
- a te sîbi — a da bir cu fugițiile
- a te vrubi — a înțelege
- a ti zdarovi cu pațanii — a te saluta cu băieții
- a ti abajri — a mînce pe săturate
- a ti prasfiti — a te da de gol
- ⚠ a-ți înfla hreașca — a bea fără măsură
- ⚠ a trisi pisiulika — (vulgar, anatomic)
- așela ari o tacikă pantovaia — are o mașină bună
- a ușidi vaprosuri — a găsi soluții
- a vîjîvi — a supraviețui
- a zaboli — a te îmbolnăvi
- ⚠ a zaibi narodu / populația — a juca pe nervi
- a zamuti șeva — a pune ceva la cale
- a zakalibi — a juca pe nervi
- ai svet acasă? — ai electricitate acasă?
- ai șeva havcic? — ai ceva de mîncecare?
- abșceagă — cămin studențesc
- abjorik — gurmand
- așăiană — în felul acesta
- am pierdut orientirovca — m-am rătăcit
- am praskaliznit — am alunecat
- ahreneti — a rămîne uimit de supărare
- anigdoati — bancuri
- astanovkă — stație
- atstîlo — s-a răcit
- atmazkă levaia — scuză ieftină
- ⚠ azabocenîi — obsedat

## B
- babă miniatiurnaia — femeie slabă
- bașleovî — om cu bani
- batoane — franzele
- ⚠ bahani / bahanu — părinți (peiorativ)
- bazaru-i mort / bazar ubit — discuția e încheiată
- bazaruri — discuții
- ⚠ bă, io’ hui io’ znait — nu știu ce să zic
- băi, vorghesc din marșutcă — vorbesc din maxi-taxi
- ⚠ blea — (interjecție vulgară)
- ⚠ băi, duracule — prostule
- bispridel — dezordine
- bîdlan — băiat rău
- bicsă — domnișoară
- bîc — bou
- buburuznik — mașină mică și proastă
- butilkă golaia — sticlă goală
- badîgă — local, cafenea
- blatnoi — șmecher
- bratanu neu — fratele meu
- bumajnik — portmoneu
- buza — (interj.) închide gura

## C
- cadru — om haios
- caloș bîhliț — încălțăminte care miroase urît
- ciainik — băiat prost
- cîcășios — băiat nerespectuos
- cărăbuș — carabinier
- cicani — Ciocana (cartier)
- ciort io’ znaiu — cine-o știe
- ciotka — bun, fain
- ciuvac — un tip oarecare
- ciuvacu șela glazești la noi — tipul (neagreat) se uită la noi
- ciuvacu șela umblă așa dutîi — se crede șmecher
- ⚠ ciumadan / ciumadan la cap — om pierdut / băiat prost
- chirpidin — unealtă de scos cuie
- chișior — picior
- ⚠ chișăt — urină
- chipitoc — apă fierbinte
- ⚠ clenomițîn — (vulgar, anatomic)
- clicuhă — poreclă
- ⚠ clopoțălcă — virgină
- ⚠ cobră — (vulgar, anatomic)
- crivaraz — persoană urîată și rea
- cum cu jituha? — ce mai faci?
- curji fesu — dus cu pluta
- curva de mare — sirenă
- cojîncă — geacă de piele

## D
- da vdrug — poate, poate merge
- davai buhneom — hai să-ți fac cinste cu ceva de băut
- dă-ni kastrulea șeea — dă-mi cratița
- dă-ni meaciu — dă mingea
- dapu kaneșna — sigur că
- devcionși — domnișoare
- dictionarul e băț — dicționarul e super tare
- dneară — zi de naștere

## E
- ea ne govoriti po ruski — nu vorbesc rusă
- el ma cumărește — nu-mi place de el
- ești un mujic bun — ești băiat de treabă
- ești puteovî? — ești deștept?
- ești șei mai kleovaia teolkă — declarație de dragoste
- eu deamu psihuiesc — devin foarte nervos
- eu mă kiș kipitok — sunt îndrăgostit pînă peste cap
- eu napastoi cîștig — sunt norocos
- eu îs bronitank — sunt cel mai tare

## F
- fa, vrei morojenî? — dragă, vrei o înghețată?
- fără slejcă — fără a fi urmărit
- fără opredeliteli — nedeterminat
- foarte horoșii — foarte bun

## G
- ⚠ ganduras — boule
- ⚠ gandon ibucii — persoană enervantă
- ⚠ gandonu șela di Vaseok nu puni buhalî — (băiat nesuferit) nu face cinste
- ganeoj — minciuni
- ghenii — băiat deștept
- goroscop — horoscop
- gura-n ghips — liniște!

## H
- hai la ozeră — hai la iaz
- hai și ni-om razbiri — hai să clarificăm
- hanîgă — om prost
- hată — casă
- huineaua șeia — ceva necunoscut

## I
- iaca (tu) — în schimb (tu)
- ibală — mutră
- ⚠ ibati — (înjurătură)
- ⚠ io maio — (înjurătură)
- ⚠ iobanîi v rot — (înjurătură)

## J
- jeleznî pareni — băiat rezistent
- ⚠ jâvotnoe — (adresare) animalule
- jgut — lănțișor
- jopă — posterior
- jruhă / jratva — mîncecare
- juc — băiat descurcăreț
- juvkă — gumă de mestecat

## K
- kaif — a te simți bine
- ⚠ kalhoz — orgie / gospodărie țărănească
- kanciugă — om terminat
- kapilkă — pușculiță
- kazeol — tip care nu vrea să facă nimic pentru tine
- kent — prieten / băiat cu influență
- kirojși — coltunași
- kitaiskaia huita — obiect produs în China
- klasna — plăcut
- kleovî — ca lumea
- koncenî — terminat, vai de capul lui
- krasovși — adidași
- krugavaia — rond
- krîșă — apărători de poziție
- krîsă — spion

## L
- lașara — un ratat
- l-o zavalit — l-au bătut
- loh — fraier
- lahadron — fraier cu experiență

## M
- m-ai ubidit — m-ai convins
- m-am vliubit — m-am îndrăgostit
- m-am udivit — m-am mirat
- ⚠ m-am râs pizdata — am râs din toate puterile
- m-am scoborat — am coborât
- m-am hodihnit — m-am odihnit
- m-o apucat sracika — am diaree
- maloletși — adolescente
- mă cișăște — mă mînce
- mamașa coași pîni — mama coace pâine
- mă duc sî ablamesc teolka — mă duc să-i fac curte
- mă tașcesc — stare euforică
- mă prițipesc — insist, agăț
- mă klabasesk în trolik — umblu brambura
- ma sohnesc dupa tine — mă usuc după tine
- maladeț — de treabă
- morda — față neagreată
- musar — polițist
- mrazi — nesimțit
- mujîc gareacii — tip sexy
- mutka — activitate ilegală

## N
- n-am vozmojnosti — nu am posibilitatea
- n-am svet — nu am curent
- na fseakii slucii — pentru orice eventualitate
- nacialinic — șef
- nastraenii — dispoziție
- ne-am tusuit — ne-am distrat
- ne-am paznakomlit — am făcut cunoștință
- ne imeet naznacenie — nu contează
- ⚠ ni-a kisă di nii amar în gurî — (vulgar, fiziologic)
- ni-a pităni uăi — mi-i foame
- ni-o dolojî — mi-a spus ce se întîmplă
- ni-o atpadit celestea — am rămas cu gura căscată
- ni-i da lampaciki — nu mă interesează
- nu fași nișio dvijenii — nu faci nimic
- nu știu și s-o slucit — nu știu ce s-a întîmplat
- nu mă vîră în tupic — nu mă dezorienteza

## O
- odușîvleonî — inspirat
- oknă — fereastră
- oleni — prost
- ostanovite rutieru u semafora — opriți maxi-taxi la semafor
- otvicești za bazar? — ești sigur de ce zici?

## P
- padruji / padruga — domnișoare / prietenă
- ⚠ padruga șeia-i koncinaia — tipa e nașpa
- parașî — WC
- pațan — băiat de-al nostru (pune de băut)
- pațan puteovii / v teme / svoi — băiat de-al nostru, bazat
- pațan ciotkos — băiat de treabă
- pațan krutoi — tip șmecher
- paȝanu șela glazești la noi — băiatul (agreat) se uită la noi
- perezagruzcă — repornire / restart
- perezaredești — reîncarcă
- pesni — cântec
- ⚠ pidaras — (slur) persoană nesuferită
- piciatkă — inel de aur
- pituh — fraier
- pivasik / pivă — bere
- ⚠ pizdeț — uimitor / treabă foarte rea (după context)
- priz — premiu
- pragon — minciună
- puteovii — bun
- ⚠ pohui / pohuist — indiferență / persoană indiferentă
- pricol / pricoluri — glumă / glume
- ⚠ prașmandovcă — prostituată
- pricioșcă — frizură
- pridurak — om prost
- ⚠ prost di put colțunii — superlativ pentru prost
- primblare — plimbare

## R
- rajdak — zi onomastică
- (a ti) raslaghi / raslabi — a te relaxa
- razisnește și nouă bazaru — lămurește-ne ce ai vrut să zici
- rîbkă — peștișor
- (la) rîbalkă — la pescuit
- rîlă / rojî — față urîată / mutră
- rjac — râs în hohote
- ⚠ rupi bășina / buca — tare beton
- rojî znakomaia — mutră cunoscută
- rukapașkă — arte marțiale (corp la corp)

## S
- s-o kâkat la delî — i-a fost teamă / a renunțat
- sabirești — a strînge
- salabon — persoană slabă, neajutorată
- samaleot — avion
- sased — vecin
- să ne kentuim — să ne împrietenim
- sî trisește — tremură / zguduie
- se grăghești — se grăbește
- semși / semiară — semințe de floarea-soarelui
- siliodkă (kapcionită) — scumbrie (afumată)
- sfitilinic — veioză
- slidește (după bazaru tău) — urmărește / fii atent ce spui
- sobutîlnîk — partener de pahar
- stariși — părinți
- stratăghie — strategie
- ⚠ stervă — ușuratică
- stucește — pîrăște
- storoj — paznic
- strelkă — întîlnire de afaceri
- ⚠ sucă — (vulgar) cățea / femeie insuportabilă
- supărat concretno — foarte supărat

## Ș
- șăzi ăi răși — calmează-te
- și s-o slucit cu tini? — ce ai pățit?
- ⚠ și ti vîibești? — te spargi în figuri?
- șibionkă — pietriș
- șiboti / șuzuri — cizme / încălțăminte
- șinși padhoadi la turnic — 5 serii la bară
- șini ne peredal za praezd? — cine n-a achitat taxa?
- șnuroc — șiret
- ștampovkă — produs falsificat
- ștucă — o mie

## T
- taburetkă / tazic — mașină mică / automobil
- talpomițîn — miros de picioare
- taman cînd a-s mă duc — cînd o să plec
- tăt o să șie clasna / tăt îi băț — totul va fi bine
- tecikă — zile critice / ciclu
- teolkă — fată sexy
- tifoznî — om bolnav
- tipa — aproximativ / similar cu
- tocîcă — punct
- tormozești aiși — opriți aici
- trudnî — greu
- tralivaz / trolik — tramvai / troleibus
- tufta — obiect nefolositor

## Ț
- ț-o ahuit cabina? — nu ai pic de respect?
- țăntru — centrul orașului
- țîmburuc — mititel
- ⚠ țîțoderjadeli — sutien

## U
- uăi / uăi fă — adresare (bărbat / femeie)
- ⚠ uăi, dacî mănănși huineaua — (avertisment vulgar) nu mănca, e toxic
- uăi, slidește bazarul — vezi că vorbești aiurea
- uăi, tu și ești kaciok? — mergi la sala de forță?
- ⚠ uăi, tu și vrei pizdealî? — (amenințare) vrei bătaie?
- uciastok — secția de poliție
- ⚠ ucilkă (ibanutaia) — profesoară (severă)
- udaci — succes
- ⚠ udavșic — (vulgar, anatomic)
- urod — persoană urîată
- umnî — deștept
- ujasno și-i di prost — prostie la extrem
- utiug — fier de călcat
- udocică — undiță
- uvajănie / uvajuhă — respect

## V
- vabșe — în general
- ⚠ vafli — suplimente (proteine) / (vulgar)
- vkliucește / vâkliucește svetul — aprinde / stinge lumina
- vnature — cu adevărat
- voobșe șî îndiobști — mai pe scurt
- vozmojnosti — posibilitate
- vrei sî-ț zamacesc una? — vrei să te lovesc?
- vreu să pazdravlesc — vreau să felicit
- vunderkind — foarte deștept

## Z
- zadrot — tocilar
- ⚠ zaebisi — minunat
- zavidenii — local
- zub daiu — îți jur!
- zapas — rezervă
- zapravkă — stație de alimentare
- zakuresc — aprind o țigară
- zașnurui șnuroașili — a lega șireturilor
- zdviguri — dus cu pluta
- zubilili — dinții
- mă belesc la zviojdi — mă uit la stele
- zvuk — sunet
